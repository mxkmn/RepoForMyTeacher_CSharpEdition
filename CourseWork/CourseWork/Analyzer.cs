/* 
 * Это попытка сделать из говнокода https://github.com/m4r1vs/Audioly хоть что-то
 * адекватное, но либа не имеет нормальной документации, так что всё очень плохо
 * (хоть этого и не заметить с первого раза) - возможно ненадёжно, неясно как
 * работает, поток берётся с самого первого аудиодевайса в системе, поскольку все
 * попытки помеять девайс заканчиваются исключительно ошибками.
